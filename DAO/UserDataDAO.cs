    ...
    class UserDataDAO
    {
        public UserData VerifyUser(MySqlConnection conn, string username, string password)
        {
            ...
            try
            {
                ...
                {
                    ControllerManager.reasonCode = Common.ReasonCode.IllegalCharacter;
                    return null;
                }
                ...
                if (reader.Read())
                {
                    ControllerManager.reasonCode = Common.ReasonCode.None;
                    UserData user = new UserData(username, password);
                    return user;
                }
                else
                {
                    ControllerManager.reasonCode = Common.ReasonCode.UsernameOrPwdError;
                    return null;
                }
            }
            catch (Exception e)
        ...
        public bool AddUser(MySqlConnection conn, string username, string password)
        {
            ...
            {
                ControllerManager.reasonCode = Common.ReasonCode.DatabaseException;
